...
int PLTHook::Impl::ReplaceFunction(const char* func_name, void* new_func, void** old_func_out) {
    ...
    while (EnumerateSymbols(pos, name, addr) == SUCCESS) {
        ...
        if (strncmp(name, func_name, name_len) == 0 && (name[name_len] == '\0' || name[name_len] == '@')) {
            // 匹配√
            ...
            // 从页首地址开始)
            void* page_addr = reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(addr) & ~(page_size_ - 1));
...
int PLTHook::Impl::LoadMemoryProtections() {
    ...
    while (fgets(buf, PATH_MAX, fp)) {
        ...
        memory_protections_.push_back({static_cast<uintptr_t>(start), static_cast<uintptr_t>(end), protection});
    }
    fclose(fp);
    return SUCCESS;
}
