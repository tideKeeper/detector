...
struct PLTHook::Impl {
  ...
  // 枚举符号表中的所有符号
  // 后缀加out说明是要传递出去的
  int EnumerateSymbols(size_t &pos, const char *&name_out,
                       void **&addr_out) const;
...
// 在动态段表中查找指定类型的表项
// 服务于InitializeFromLinkMap函数, 通过link_map获取到动态段表的首地址,
// 然后在动态段表中查找指定类型的表项
...
// 底层原理和printf 类似,
// 通过可变参数列表来格式化字符串,只不过这里是存储到last_error_中,
// 方便后续获取错误信息
...
// 构造函数, 初始化impl, 这个构造函数是被私有化的,
// 需要在create静态函数中调用获取 主要是把link_map传入impl中并进行构造,
// 让impl去初始化所需的表信息
...
std::unique_ptr<PLTHook> PLTHook::Impl::Create(const char *file_name) {
  ...
  if (!file_name) {
    ...
    // 获取到主程序的link_map后, 需要遍历到最前面的link_map,
    // 因为主程序的link_map可能是链表的最后一个节点 而第一个元素则是主程序, 其在
    // link_map 链表里的标识方式和共享库不一样，不能像普通.so 那样
    // “按名字精准查询”，所以才用「找链表头部」的方式来定位它.
    while (lmap->l_prev != nullptr) {
      lmap = lmap->l_prev;
    }

    // 总流程: create(filename) -> link_map -> new PLTHook(lmap) -> new
    // Impl(lmap) -> 初始化各类信息
...
// 替换函数
int PLTHook::ReplaceFunction(const char *func_name, void *new_func,
                             void **old_func_out) {
  return pimpl_->ReplaceFunction(func_name, new_func, old_func_out);
}

int PLTHook::Impl::ReplaceFunction(const char *func_name, void *new_func,
                                   void **old_func_out) {
  // 入参合法性校验
  ...
  while (EnumerateSymbols(pos, name, addr) == SUCCESS) {
    ...
    if (strncmp(name, func_name, name_len) == 0 &&
      ...
      // 计算包含该地址的页的起始地址, 用于修改页的内存权限(必须以页为单位,
      // 从页首地址开始)
      ...
      if (!guard.IsOk()) {
        SetError("Could not change memory protection at %p: %s", page_addr,
                 strerror(errno));
        return INTERNAL_ERROR;
      }
...
const std::string &PLTHook::GetLastError() { return Impl::last_error_; }

/*↑ ======PLT的接口完善======== ↑*/

// impl构造函数
PLTHook::Impl::Impl()
    : dynsym_(nullptr), dynstr_(nullptr), dynstr_size_(0),
      plt_addr_base_(nullptr), rela_plt_(nullptr), rela_plt_cnt(0) {}
...
// 从这个构造也可以看出impl最主要的作用, 就是初始化所需的表信息,
// Load内存保护的信息, 具体的工作函数, 其实这些都是具体的工作函数hhh.

int PLTHook::Impl::InitializeFromLinkMap(struct link_map *lmap) {
  // 设置基地址, 这是共享库加载到内存中的基地址
  // lmap是一个综合性的指针, 需要对里面的内容解引用,
  // 这里就是用plt_addr_base_来存储基地址(l_addr)
  ...
  // 用dyn作临时变量,
  // 当FindDynamicEntry()成功后再对动态表中的各类符号指针进行赋值
  // 获取动态段中的符号表
  ...
  rela_plt_ =
      reinterpret_cast<const Elf64_Rela *>(base_offset + dyn->d_un.d_ptr);
...
int PLTHook::Impl::LoadMemoryProtections() {
  ...
  while (fgets(buf, PATH_MAX, fp)) {
    ...
    memory_protections_.push_back({static_cast<uintptr_t>(start),
                                   static_cast<uintptr_t>(end), protection});
  }
  fclose(fp);
  return SUCCESS;
}
...
// 枚举SymTable中的所有符号
// 后缀加out说明是要传递出去的
int PLTHook::Impl::EnumerateSymbols(size_t &pos, const char *&name_out,
                                    void **&addr_out) const {
  // 遍历rela_plt_cnt次
